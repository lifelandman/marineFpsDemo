...
def make_client_player_class(base_class = playerEnt):
    class newClientPlayer(base_class):
...
clientPlayer = make_client_player_class()
...
def make_host_net_player_class(base_class = playerEnt):
    ...
    return newHostNetPlayer

hostNetPlayer = make_host_net_player_class()
...
def make_client_net_player_class(base_class = playerEnt):
    class newClientNetPlayer(base_class):#This one doesn't check to see if movement seems legit nor sends out instructions.
        ...
        def set_weapon(self, slot, priority):
            self.wpnMgr.goto_subSlot(slot, priority)
    return newClientNetPlayer

clientNetPlayer = make_client_net_player_class()